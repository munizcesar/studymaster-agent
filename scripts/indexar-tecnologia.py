...
CONTEUDOS_TECNOLOGIA = [
  ...
  {"id":"tec-web-07","area":"Tecnologia","disciplina":"Desenvolvimento Web","namespace":"tecnologia_geral",
   "texto":"Tecnologia Desenvolvimento Web — WebAssembly: WebAssembly (Wasm) é um formato de bytecode portátil e seguro para execução no navegador e no server-side. Compila C/C++, Rust e outras linguagens para módulos binários de alto desempenho que interagem com JavaScript via API. Wasm oferece performance próxima do nativo em cálculos intensivos, jogos, edição de imagem, realidade virtual e codecs, mantendo sandboxing e interoperabilidade. Ferramentas como Emscripten, wasm-pack e WASI estendem seu uso além do browser."},
