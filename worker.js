...
${optionsExampleText}
      ...
      "correctAnswer": "C",
      "explanation": "explicação completa citando literalmente o artigo de lei, doutrina, teoria ou conceito que fundamenta a resposta. Ex: Art. 5º, CF/88 ou Segunda Lei de Newton (F = m·a)",
...
**👉 O que você prefere?**` };
  ...
  try {
    const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: modelOverride || 'llama-3.1-70b-versatile',
        messages,
        temperature: 0.35,
        max_tokens: 2000,
...
8. Confidence: Registre um nível de confiança ("Alta", "Média", "Baixa") para cada item.`;
          ...
          const modelsToTry = ['llama-3.1-70b-versatile', 'llama-3.1-8b-instant'];
          let response;
          let errText = '';
...
if (mode === "academic") {
  ...
  async queue(batch, env) {
    ...
    for (const msg of batch.messages) {
       try {
         if (batch.queue === 'studymaster-discovery-queue') {
            await pipeline.processDiscoveryQueue(msg);
         } else if (batch.queue === 'studymaster-fetch-queue') {
            await pipeline.processFetchQueue(msg);
         } else if (batch.queue === 'studymaster-extraction-queue') {
            await pipeline.processExtractionQueue(msg);
         } else if (batch.queue === 'studymaster-metadata-queue') {
            await pipeline.processMetadataQueue(msg);
         } else if (batch.queue === 'studymaster-ingest-queue') {
            msg.ack();
         } else {
            msg.ack();
         }
       } catch (err) {
         console.error(`Queue error for ${batch.queue}:`, err);
         msg.retry();
       }
    }
  },
